...
class Weather(Producer):
    ...
    def run(self, month):
        ...
        try:
            ...
            logger.info(f"sent weather data to to REST Proxy, temp: {self.temp}, status: {self.status.name}")
            logger.debug(f"received response {json.dumps(resp.json(), indent=2)} ")
        except:
